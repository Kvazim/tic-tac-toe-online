...
export function UiModal({ width="md",className, children, isOpen = false, onClose }) {
  ...
  const modal = (
    <div className={clsx("fixed inset-0 overflow-y-auto bg-slate-900/60 backdrop-blur pt-10 pb-10", className)} onClick={handleClick}>
      <div data-id="modal" className={clsx(
        ...
        <button className="w-8 h-8 rounded flex justify-center items-center  absolute top-0 left-[calc(100%+12px)] bg-white/10 hover:bg-white/40 transition-colors" onClick={onClose}>
...
UiModal.Footer = function UiModalFooter({children, className}) {
  return (
    <div className={clsx(className, "mt-auto p-6 flex items-center justify-end gap-4")}>{children}</div>
  );
};
